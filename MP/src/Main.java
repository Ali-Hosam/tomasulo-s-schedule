...
//    in the same clock cycle
    ...
    static int[] integerRegisters=new int[20];
...
//        store_buffer_size= sc.nextInt();
    ...
    private static void issueInstruction(Object[][] aRs, int availablePlace, String[] slots, char type) {
        ...
        if(tag.contains("S"))
            System.out.println("the store is put");
        if (tag.contains("L"))
...
//            else loadBuffer[availablePlace][4]=regReservations[destReg];
        ...
        } else if (typeOfInstruction.toLowerCase().charAt(0) == 's') {
            slots[1].trim();
...
//            System.out.println("reserved the store register");
            ...
            //the effective address
            int address = Integer.parseInt(slots[1].trim());
            System.out.println("hi the store address is: " + address);
...
//                break;
            case 'S':
                System.out.println("3edkom sa3eed");
        }
...
//            System.out.println("value in the corresponding placeholder in loadBuffer: "+loadBuffer[index][2]);
        ...
        } else if (tag.toLowerCase().contains("s")) {
            ET = storeET;
//            System.out.println("el execution ");
            //TODO:after constructing the cache the value of storeBuffer[index][2]should be stored in
            // the address (storeBuffer[index][3]) in the cache
            /*cache[storeBuffer[index][3]]*/

//            storeBuffer[index][3] = 101;
            cache[(Integer) storeBuffer[index][3] % cache.length].value = ((Float) storeBuffer[index][2]).byteValue();
            memory[(Integer) storeBuffer[index][3]] = ((Float) storeBuffer[index][2]).byteValue();
            System.out.println("address in cache: " + (Integer) storeBuffer[index][3] % cache.length + "\n"
            + "address in memmory" + (Integer) storeBuffer[index][3]);
            System.out.println("store hna ya 7ob");
//            ET=storeET;
//            //loading the value in the dest. register
...
//    instructions.remove();
                ...
                if (slots[0].toLowerCase().charAt(0) == 'd') {
//                    String operation=slots[0].substring(1);
                    issueIntegerOperation(slots);
                    instructions.remove();
                } else {
...
//                        address = 100;//addressRegisters[registerNumber];
                            ...
                            System.out.println("address slot for store: " + address);
                            instructions.remove();
                            System.out.println("issued the inst store at: " + clockCycle);
//                        System.out.println("the 2nd slot is: "+slots[0]);
...
//            break;
    ...
    private static void issueIntegerOperation(String[] slots) {
        String []parts=slots[0].split(" ");
        System.out.println("1st: "+parts[1]);
            int destination=Integer.parseInt(parts[1].substring(1));
        System.out.println(destination);
            int operand=Integer.parseInt(slots[1].trim().substring(1));
        System.out.println(operand);
            int immediate=Integer.parseInt(slots[2].trim());
        System.out.println(immediate);
        if((slots[0].substring(1)).toLowerCase().contains("add")){
            integerRegisters[destination]=integerRegisters[operand]+immediate;
            System.out.println("integer op: "+integerRegisters[destination]);
        }
        if(slots[0].toLowerCase().contains("sub")){
            integerRegisters[destination]=integerRegisters[operand]-immediate;
            System.out.println("integer op: "+integerRegisters[destination]);
